...
正式执行入口是 [TMTM 实现](../src/ramtm/baselines/tmtm.py) 和 [ST-MTM 实现](../src/ramtm/baselines/stmtm.py)。旧代理不再参加本轮主比较。
...
**方法名称：RA-MTM — Reference-Anchored Merge Tree Maps（参照锚定合并树图）。**这是当前工作的命名，未作名称独占性或文献首创断言。名称突出固定真实空间参照；绝对量编码、运动残差和可行域误差预算是其配套机制。历史结果中的 `Budget`／“本文预算方法”均指同一个布局核心。完整输出入口为 [reference_anchored_merge_tree_maps.py](../src/ramtm/reference_anchored.py)。

代码：[error_budget.py](../src/ramtm/error_budget.py)。保留既有原型的研究思路，去掉硬编码三叶与代理 baseline，改为真实层次输入、小树合法序枚举和独立可行性证书。
...
生成器：[datasets.py](../experiments/synthetic_1d/datasets.py)。源参数域为 `s=0,0.2,…,120`，601个固定样本，邻接始终为 `(i,i+1)`。每帧通过指定控制点的分段线性插值得到真实标量值；用一维 join-tree sweep 提取增广树。
...
另有三个微型算子检查：重合质心时 inverse 权重无定义但 uniform 仍可运行；4个单点离散区间若要求3个空隙至少需要7像素；本文两个宽度6加间隔0.5无法放入长度5画布。它们不参与主方法排名，不被用于宣称“所有方法都会失败”。细节见 [boundary_checks.csv](../results/synthetic_1d/tables/boundary_checks.csv) 与 [verification.json](../results/synthetic_1d/records/verification.json)。
...
身份通过提取出的峰与已知生成峰中心做一对一最近匹配，三个方法使用同一对应；生成中心只用于匹配和解释控制变量，参考q及面积均从叶支持重新计算。所有输入NPZ、树弧JSON和生成参数在本地保存于 `data/generated/gaussian_2d`，不上传；目录约定见 [数据说明](../data/README.md)。离散采样与鞍点变化意味着“连续模板纯平移”下离散支持量也可能小幅变化，因此真值使用实测支持，而不强设面积不变或位移恰为12。
...
入口：[experiments/gaussian_2d/run_experiment.py](../experiments/gaussian_2d/run_experiment.py)。三组使用同一套参数，未逐场景调参。RA-MTM 使用画布120、面积比例c=0.012、gap=0.5、ρ=0.5、Δ=1、λ=0.5、η=0.1。ST-MTM 使用 uniform 距离权重、r=0.95、λ=0.5、α=1、δ=0.01，固定 `K=0.012×首帧实际总叶面积`；ST-MTM 和 RA-MTM 均用2048像素填充，TMTM保留论文全样本语义，输出4225样本。这是机制验证，未冒充相同输出预算的系统公平评测。
...
RA-MTM本组三个锚点的参考误差分别处于固定±1预算边界，因此位置MAE=1，而跨帧误差残差几乎不变，运动MAE接近机器零。这来自所设计的约束与场景，不是独立证明复杂时间正则必要；其必要性仍应在有噪声／层次变化消融中检验。固定x投影也不保全二维距离：G3 ST-MTM的距离NRMSE为0.0362、RA-MTM为0.2031。[配套stress曲线](../results/gaussian_2d/figures/geometry_stress.png)保留这一差异。应把优势限定为研究任务要求的空间参照和绝对量语义，不写成全指标优胜。
...
所有消融使用同一合法序枚举、LP预算和求解流程，只改变指定机制，16组结果完整保存在 [ablation.csv](../results/synthetic_1d/tables/ablation.csv)。
...
- 117项解析／回归检查通过，结果见 [verification.json](../results/synthetic_1d/records/verification.json)。这些是测试项数量，不是117个独立统计样本。
...
python experiments/synthetic_1d/finalize.py
```

脚本默认使用本机已存在的 `python3`。也可设置 `PYTHON_INITIAL=/absolute/path/to/python`，按项目根目录的 [pyproject.toml](../pyproject.toml) 安装依赖后运行。不会下载真实数据、发送网络请求或改写本报告；重新生成数值、图、验证结果和最终哈希。跨机器图形元数据／运行时间可能不同，数值按给定容差核对。

```text
docs/EXPERIMENT_REPORT.md         本报告，唯一活动说明文档
references/                             两篇原始PDF、来源和哈希
src/ramtm/baselines/                          两个核验修正后的唯一正式baseline
src/ramtm/error_budget.py              RA-MTM布局核心
src/ramtm/reference_anchored.py  命名入口与固定参照标量渲染
data/generated/synthetic_1d/        一维／折线输入与树
data/generated/gaussian_2d/         二维高斯场输入与树
data/real/                         原始ERA5、744份CN-Reanalysis CSV及既有环境
experiments/synthetic_1d/                 原机制实验、消融、扫描和收尾脚本
experiments/gaussian_2d/              二维高斯生成、三方法运行、完整图验证与绘图
results/synthetic_1d/                     一维／折线数值、证书、中间结果及PNG/SVG
results/gaussian_2d/                  二维CSV、NPZ、中间JSON、PNG/SVG与GIF
archive/                            旧版本可恢复快照、旧材料、作者源码参考
数据 -> data/real                   兼容已有Python环境路径的符号链接
```

输出按“场景名＋方法名”一一对应：`*_TMTM.npz`、`*_STMTM.npz`、`*_Budget.npz`。原始输入可由 `data/generated/synthetic_1d/<场景>.npz` 和 `<场景>_trees.json` 追溯；D8只导出摘要，无树JSON。

关键结果入口：

- **新增二维主图：**[原场＋三种方法完整对比](../results/gaussian_2d/figures/translation_growth_comparison.png)、[动态图](../results/gaussian_2d/figures/translation_growth.gif)。
- [主要对比图](../results/synthetic_1d/figures/comparison.png)、[机制与完整baseline图](../results/synthetic_1d/figures/diagnostics.png)。
- [全部定量指标](../results/synthetic_1d/tables/metrics.csv)、[逐帧轨迹](../results/synthetic_1d/tables/trajectories.csv)、[适用性状态](../results/synthetic_1d/tables/applicability.csv)。
- [LP下界](../results/synthetic_1d/tables/lp_lower_bounds.csv)、逐场景 `*_budget_certificates.json`，包含每个候选序下界、选中序、约束余量及QP间隙证书。
- [拓扑检查](../results/synthetic_1d/tables/topology_checks.csv)、[验证总表](../results/synthetic_1d/records/validation_summary.json)、[参数扫描](../results/synthetic_1d/tables/sensitivity.csv)、[扰动检查](../results/synthetic_1d/tables/centroid_perturbation.csv)。
- [消融结果](../results/synthetic_1d/tables/ablation.csv)、[最终文件哈希](../results/synthetic_1d/manifest.json)。
