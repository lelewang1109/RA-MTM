"""RA-MTM dual-axis experiments on Ring and ERA5 with baseline and reference ablations."""
...
def execution_sources():
    ...
    paths += [ROOT/p for p in ['experiments/public.py',
        'experiments/run.py','experiments/era5.py',
        'experiments/ring/dataset.py','experiments/ring/source/SpreadingRingGeneration.py']]
...
def plot(name,sc,runs,span,folder,metrics,reference_kind="centroid"):
    targets=reference_points_from_frames(sc["frames"],sc["ids"],kind=reference_kind)
    ...
    if name=='era5':
        ...
        ep.save(folder/'color_style.json',dict(palette='pressure_soft',source='RA-MTM history 5a5d6d5: experiments/real_era5/paper_figure.py',
    ...
    for ax,t in zip(axs,np.linspace(0,len(times)-1,5).astype(int)):
        im=ax.pcolormesh(sc['coords'][:,0].reshape(sc['fields'][t].shape),sc['coords'][:,1].reshape(sc['fields'][t].shape),sc['fields'][t]-datum,shading='nearest',vmin=lo,vmax=hi,cmap=cmap)
        ax.set(xlim=(0,span),ylim=(0,span),aspect='equal')
    ...
    for k,m in enumerate(['X-only RA-MTM','Dual-Y']):
        ...
        ax.set(title=('RA-MTM: X (shared with X-only)' if k==0 else 'RA-MTM: Y')+f" ({len(r['maps'])} samples)",xlabel='Time step',ylabel=('X' if k==0 else 'Y')+' reference coordinate')
    fig.suptitle(name.upper()+f': baselines and RA-MTM {reference_kind}-reference maps\nBackground = scalar values; no identity dots; fixed world coordinates',fontsize=12)
    ...
    for col,tid in enumerate(ids):
        for axis in [0,1]:
            ...
            truth=[targets[t][i,axis] for t,i in zip(tt,ix)];rr=runs['X-only RA-MTM' if axis==0 else 'Dual-Y']['rows']
            axs[axis,col].plot(tt,truth,'k--',label='Target '+reference_kind)
            axs[axis,col].plot(tt,[rr[t]['x'][i] for t,i in zip(tt,ix)],color=['#0072b2','#d55e00','#009e73'][col],label='Optimized anchor')
...
def main(name,reference_kind="extremum",output_root=OUT):
    if reference_kind not in ("centroid","extremum"):raise ValueError("unknown reference kind")
    folder=output_root/name;folder.mkdir(parents=True,exist_ok=True);ep.save(folder/'status.json',dict(status='running'))
    start=time.perf_counter()
    if name=='ring':
    ...
    targets=reference_points_from_frames(sc["frames"],sc["ids"],kind=reference_kind)
    evaluation_scene=dict(sc,centers=targets)
    preprocess=time.perf_counter()-start
    ep.save(folder/'protocol.json',dict(dataset=name,source=provenance,extraction=sc.get('protocol'),parameters=asdict(p),execution_sources=execution_sources(),
        reference=reference_kind,geometry='support-centroid 2-D pairwise distances',
        origin='fixed world canvas lower-left (0,0); ERA5 geographic rectangle is padded in its fixed projected square',
    ...
    ep.save(folder/'shared_features.json',dict(reference_kind=reference_kind,reference_points=targets,centers=sc['centers'],areas=sc['areas'],tracks=sc['tracks'],leaf_ids=sc['ids'],hierarchies=sc['hier'],dates=sc['dates'],tracking=sc['tracking']))
    runs={};timings=[];view_methods=['TMTM','ST-MTM','X-only RA-MTM','Dual-Y']
    if reference_kind=='extremum':view_methods+=['Centroid-X','Centroid-Y']
    for rep in range(3):
        for m in view_methods[rep:]+view_methods[:rep]:
            ...
            else:r=solve_view(sc,p,0 if m in ['X-only RA-MTM','Centroid-X'] else 1,nominal,
                              reference_points=sc['centers'] if m.startswith('Centroid-') else targets)
    ...
    methods=[('TMTM','TMTM',None),('ST-MTM','ST-MTM',None),
             ('X-only RA-MTM','X-only RA-MTM',None),('RA-MTM','X-only RA-MTM','Dual-Y')]
    if reference_kind=='extremum':methods += [('X-only centroid RA','Centroid-X',None),('Dual centroid RA','Centroid-X','Centroid-Y')]
    for m,xkey,ykey in methods:
        dual=ykey is not None;r=runs[xkey];y=runs[ykey] if dual else None
        qm=quality(sc,r['rows']);ym=quality(sc,y['rows']) if dual else None
        met=position_motion_metrics(targets,positions(evaluation_scene,r['rows'],None if y is None else y['rows']),sc['tracks'],(span,span),.001*span)
        table.append(dict(dataset=name,method=m,**met,**{k+'_x':v for k,v in qm.items()},**{k+'_y':None if ym is None else ym[k] for k in qm},
            runtime_seconds=float(np.median([sum(v['layout_render_validation_seconds'] for v in timings if v['repetition']==rep and v['method'] in ([xkey,ykey] if dual else [xkey])) for rep in range(3)])),raster_x=len(r['maps']),raster_y=len(y['maps']) if dual else None,
            tau_x=max(v.get('tau',0) for v in r['rows']) if 'RA' in m else None,tau_y=max(v['tau'] for v in y['rows']) if dual else None))
        rendered.append(dict(dataset=name,method=m,**position_motion_metrics(targets,positions(evaluation_scene,r['rows'],None if y is None else y['rows'],True),sc['tracks'],(span,span),.001*span)))
        if not dual:
            x0=r['rows'][0]['x'];centered=x0-x0.mean();den=float(centered@centered)
            slope=(centered[:,None]*(targets[0]-targets[0].mean(0))).sum(0)/den if den>1e-12 else np.zeros(2)
            offset=targets[0].mean(0)-x0.mean()*slope
            ...
            affine_rows.append(dict(dataset=name,method=m,calibration='first_frame_2d_affine',first_frame_identifiable=den>1e-12,
                **position_motion_metrics(targets,decoded,sc['tracks'],(span,span),.001*span)))
        pos=positions(evaluation_scene,r['rows'],None if y is None else y['rows'])
        for threshold,interior in [(0.,False),(.25,False),(.5,False),(0.,True)]:
            ...
            for link in sc['tracking']:
                ...
                errors.append(np.linalg.norm((pos[t][i]-pos[t-1][j])-(targets[t][i]-targets[t-1][j]))/(span*np.sqrt(2)))
    ...
    # Always retain both target definitions; changing the target is not an improvement claim.
    target_metrics=[]
    for target_name,truth in [('centroid',sc['centers']),('extremum',reference_points_from_frames(sc['frames'],sc['ids'],kind='extremum'))]:
        for method,xkey,ykey in methods:
            decoded=positions(dict(sc,centers=truth),runs[xkey]['rows'],None if ykey is None else runs[ykey]['rows'])
            target_metrics.append(dict(method=method,target=target_name,**position_motion_metrics(truth,decoded,sc['tracks'],(span,span),.001*span)))
    ep.table(folder/'metrics_by_target.csv',target_metrics)
    plot(name,sc,runs,span,folder,table,reference_kind)
