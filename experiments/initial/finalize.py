...
from run_initial import OUT,DATA,save_json
ver=json.loads((OUT/'verification.json').read_text())
assert all(r['status']=='pass' for r in ver['checks'])
assert ver['stmtm_topology_equal']==ver['topology_frames']
required=['comparison.png','diagnostics.png','geometry.png','metrics.csv','trajectories.csv','applicability.csv',
          'lp_lower_bounds.csv','topology_checks.csv','ablation.csv','sensitivity.csv','centroid_perturbation.csv']
assert all((OUT/f).exists() for f in required)
certificates=[]
for p in OUT.glob('*_budget_certificates.json'):
...
from run_initial import write_csv
write_csv(OUT/'boundary_checks.csv',boundary)
save_json(OUT/'validation_summary.json',dict(formula_regressions=len(ver['checks']),scalar_topology_frames=ver['topology_frames'],
...
paths=[]
for folder in ['results','data/constructed','baselines','methods','experiments','papers']:
    paths.extend(p for p in (ROOT/folder).rglob('*') if p.is_file() and '__pycache__' not in p.parts and p.name!='manifest.json')
if (ROOT/'EXPERIMENT_INITIAL_REPORT.md').exists():paths.append(ROOT/'EXPERIMENT_INITIAL_REPORT.md')
