...
from methods.error_budget import Parameters, solve_sequence, leaf_orders, InfeasibleLayout

OUT=ROOT/'results/initial'; DATA=ROOT/'data/constructed'; OUT.mkdir(parents=True,exist_ok=True); DATA.mkdir(parents=True,exist_ok=True)
P=Parameters()
...
def input_plot(scenes):
    ...
    fig.savefig(OUT/'inputs.png',dpi=180);fig.savefig(OUT/'inputs.svg');plt.close(fig)
...
def diagnostic_plot(scenes,results):
    ...
    fig.savefig(OUT/'diagnostics.png',dpi=180);fig.savefig(OUT/'diagnostics.svg');plt.close(fig)
...
def geometry_plot(scenes):
    ...
    fig.savefig(OUT/'geometry.png',dpi=180);fig.savefig(OUT/'geometry.svg');plt.close(fig)
    ...
    for sc in scenes:
        ...
        for method in ['TMTM','STMTM','Budget']:
            ...
            if method=='TMTM':r=run_b1(sc)
                ...
                save_json(OUT/(name+'_budget_certificates.json'),detail)
            ...
            np.savez_compressed(OUT/(name+'_'+method+'.npz'),**arrays)
            if method=='STMTM':save_json(OUT/(name+'_stmtm_intermediates.json'),dict(parameters=r['parameters'],padding=r['padding'],continuous=r['records'],discrete=[asdict(s) for s in r['discrete']]))
            if method=='TMTM':save_json(OUT/(name+'_tmtm_intermediates.json'),dict(decisions=r['decisions'],energy=r['energy'],initial_shift=r['shift']))
    write_csv(OUT/'metrics.csv',rows);write_csv(OUT/'trajectories.csv',tr);write_csv(OUT/'applicability.csv',statuses);write_csv(OUT/'lp_lower_bounds.csv',lp)
    input_plot(scenes);geometry_plot(scenes);comparison_plot(scenes,results);diagnostic_plot(scenes,results)
    save_json(OUT/'summary.json',dict(parameters=asdict(P),python=platform.python_version(),numpy=np.__version__,scipy=scipy.__version__,metrics=rows,statuses=statuses))
    manifest={str(p.relative_to(ROOT)):hashlib.sha256(p.read_bytes()).hexdigest() for directory in [OUT,DATA,ROOT/'baselines',ROOT/'methods',ROOT/'experiments/initial'] for p in directory.glob('*') if p.is_file() and p.name!='manifest.json'}
