...
def main():
    ...
    for name in ['translation','growth','translation_growth']:
        ...
        for m in METHODS:
            ...
            if m=='TMTM':
                r=align_orientation(sc,run_b1(sc));save_json(RECORDS/(name+'_tmtm_intermediates.json'),dict(decisions=r['decisions'],energy=r['energy'],initial_shift=r['shift'],reflection=r['display_reflection'],translation=r['display_translation']))
            elif m=='ST-MTM':
                p=b2.LayoutParameters('uniform',float(P.width_scale*sc['area'][0].sum()),.95,.5,L,0,min_spacing_delta=.01,optimizer_tolerance=1e-11)
                r=align_orientation(sc,run_b2(sc,p));save_json(RECORDS/(name+'_stmtm_intermediates.json'),dict(parameters=asdict(p),records=r['records'],discrete=[asdict(d) for d in r['discrete']],padding=r['padding'],reflection=r['display_reflection'],translation=r['display_translation']))
            else:
    ...
    save_json(RECORDS/'validation.json',dict(frames=T*3,full_map_checks=len(checks),topology_passed=sum(c['scalar_topology_equal'] for c in checks),parameters=asdict(P),grid=GRID,cell_area=CELL,layout_length=L,max_raster_anchor_error=max(e['anchor_error'] for e in raster),max_raster_interval_extent_error=max(e['interval_extent_error'] for e in raster)))
    assert all(c['scalar_topology_equal'] for c in checks),'See topology_checks.csv; do not claim full topology preservation'
    files=[p for folder in [OUT,DATA,ROOT/'src/ramtm',ROOT/'experiments/gaussian_2d'] for p in folder.rglob('*') if p.is_file() and '__pycache__' not in p.parts and p.name!='manifest.json']
    files.extend([ROOT/'docs/EXPERIMENT_REPORT.md',ROOT/'experiments/synthetic_1d/run_experiments.py',ROOT/'experiments/synthetic_1d/datasets.py'])
