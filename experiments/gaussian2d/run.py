...
Run: python experiments/gaussian2d/run.py
"""
from pathlib import Path
import sys,json,time,hashlib
from dataclasses import asdict
ROOT=Path(__file__).resolve().parents[2]
sys.path.insert(0,str(ROOT/'experiments/initial'));sys.path.insert(0,str(ROOT))
import numpy as np
...
from methods.reference_anchored_merge_tree_maps import Parameters,solve_sequence,render_sequence
from run_initial import run_b1,run_b2,pack,metrics,save_json,write_csv
OUT=ROOT/'results/gaussian2d';DATA=ROOT/'data/constructed/gaussian2d'
OUT.mkdir(exist_ok=True,parents=True);DATA.mkdir(exist_ok=True,parents=True)
GRID=65;T=21;L=2048;C=120.;CELL=(C/(GRID-1))**2
P=Parameters(width_scale=.012,extra_budget=1.,motion_weight=.5)
COLORS=['#1476ad','#de7f26','#9b57a1'];METHODS=['TMTM','ST-MTM','RA-MTM']
...
def make_scene(name):
    ...
    for t in range(T):
        ...
        controls.append(dict(t=t,shift=shift,dilation=dilation,gaussian_centers=mu,sigma=sigma))
    return dict(name='gaussian2d_'+name,values=np.array(vals),coords=coords,trees=trees,frames=frames,ids=ids,
        centers=np.array(centers),area=np.array(area),hierarchies=hierarchies,summary_only=False,controls=controls)

def align_orientation(sc,r):
    # One fixed reflection and translation only, selected at t=0. No time-dependent alignment.
    q=sc['centers'][0,:,0];x=r['x'][0];sgn=1 if np.dot(x-x.mean(),q-q.mean())>=0 else -1
    shift=q.mean()-sgn*x.mean()
    r['x']=sgn*r['x']+shift;r['z']=sgn*r['z']+shift
    if sgn<0:
        r['order']=[tuple(reversed(o)) for o in r['order']];r['scalar_map']=r['scalar_map'][::-1]
        if 'sample_positions' in r:
            r['sample_positions']=r['scalar_map'].shape[0]-1-r['sample_positions']
    r['display_reflection']=sgn;r['display_translation']=float(shift)
    return r
...
def figures(name,sc,res):
    ...
    for j,m in enumerate(METHODS):
        ...
        if m=='RA-MTM':
            for i in range(3):ax.plot(range(T),sc['centers'][:,i,0],color='white',ls='--',lw=.9,alpha=.75)
    ...
    fig.savefig(OUT/(name+'_comparison.png'),dpi=180);fig.savefig(OUT/(name+'_comparison.svg'));plt.close(fig)
    ...
    fig.savefig(OUT/(name+'_intermediates.png'),dpi=160);plt.close(fig)
...
def animation(sc,res):
    ...
    def draw(t):
        ...
        for ax,m in zip([axs[0,1],axs[1,0],axs[1,1]],METHODS):
            ...
            if m=='RA-MTM':
                for i in range(3):ax.plot(range(t+1),sc['centers'][:t+1,i,0],color='white',ls='--',lw=.8)
    ...
    anim.save(OUT/'translation_growth.gif',writer=PillowWriter(fps=4),dpi=100);plt.close(fig)

def geometry_diagnostics(allsc,allres):
    fig,axs=plt.subplots(1,3,figsize=(14,3.8),layout='constrained')
    for ax,(name,sc) in zip(axs,allsc.items()):
    ...
    axs[0].legend();fig.suptitle('Companion geometry metric: fixed x-reference does not preserve all 2-D distances')
    fig.savefig(OUT/'geometry_stress.png',dpi=180);plt.close(fig)

def main():
    rows=[];checks=[];trajectories=[];allsc={};allres={};raster=[]
    for name in ['translation','growth','translation_growth']:
        print('extract',name,flush=True);sc=make_scene(name);res={}
        ...
        save_json(DATA/(name+'_trees.json'),dict(cell_area=CELL,controls=sc['controls'],hierarchies=sc['hierarchies'],frames=[dict(root=f.root,children=f.children,arcs=[asdict(a) for a in f.arcs.values()]) for f in sc['frames']]))
        for m in METHODS:
            print('run',name,m,flush=True);start=time.perf_counter()
            if m=='TMTM':
                r=align_orientation(sc,run_b1(sc));save_json(OUT/(name+'_tmtm_intermediates.json'),dict(decisions=r['decisions'],energy=r['energy'],initial_shift=r['shift'],reflection=r['display_reflection'],translation=r['display_translation']))
            elif m=='ST-MTM':
                p=b2.LayoutParameters('uniform',float(P.width_scale*sc['area'][0].sum()),.95,.5,L,0,min_spacing_delta=.01,optimizer_tolerance=1e-11)
                r=align_orientation(sc,run_b2(sc,p));save_json(OUT/(name+'_stmtm_intermediates.json'),dict(parameters=asdict(p),records=r['records'],discrete=[asdict(d) for d in r['discrete']],padding=r['padding'],reflection=r['display_reflection'],translation=r['display_translation']))
            else:
                ...
                save_json(OUT/(name+'_ramtm_intermediates.json'),dict(parameters=asdict(P),certificates=detail,discrete=[asdict(d) for d in ds],raster_errors=er))
            ...
            row=metrics(sc,r,m,elapsed);row['total_area_ratio_final']=float(r['w'][-1].sum()/r['w'][0].sum());row['true_total_area_ratio_final']=float(sc['area'][-1].sum()/sc['area'][0].sum());row['mean_displacement_final']=float(np.mean(r['x'][-1]-r['x'][0]));row['true_mean_displacement_final']=float(np.mean(sc['centers'][-1,:,0]-sc['centers'][0,:,0]));rows.append(row)
            np.savez_compressed(OUT/(name+'_'+m+'.npz'),**{k:v for k,v in r.items() if isinstance(v,np.ndarray)})
            for t,tr in enumerate(sc['trees']):
                ...
                checks.append(dict(scene=name,method=m,t=t,scalar_topology_equal=actual==truth,source_leaf_count=len(truth[1]),output_leaf_count=len(actual[1]),hierarchy_legal=True))
                for i in range(3):trajectories.append(dict(scene=name,method=m,t=t,feature=chr(65+i),q=sc['centers'][t,i,0],area=sc['area'][t,i],x=r['x'][t,i],z=r['z'][t,i],width=r['w'][t,i]))
        figures(name,sc,res)
    ...
    write_csv(OUT/'metrics.csv',rows);write_csv(OUT/'topology_checks.csv',checks);write_csv(OUT/'trajectories.csv',trajectories)
    write_csv(OUT/'raster_errors.csv',raster)
    save_json(OUT/'validation.json',dict(frames=T*3,full_map_checks=len(checks),topology_passed=sum(c['scalar_topology_equal'] for c in checks),parameters=asdict(P),grid=GRID,cell_area=CELL,layout_length=L,max_raster_anchor_error=max(e['anchor_error'] for e in raster),max_raster_interval_extent_error=max(e['interval_extent_error'] for e in raster)))
    assert all(c['scalar_topology_equal'] for c in checks),'See topology_checks.csv; do not claim full topology preservation'
    files=[p for folder in [OUT,DATA,ROOT/'methods',ROOT/'baselines',ROOT/'experiments/gaussian2d'] for p in folder.rglob('*') if p.is_file() and '__pycache__' not in p.parts and p.name!='manifest.json']
    files.extend([ROOT/'EXPERIMENT_INITIAL_REPORT.md',ROOT/'experiments/initial/run_initial.py',ROOT/'experiments/initial/datasets.py'])
    save_json(OUT/'manifest.json',{str(p.relative_to(ROOT)):hashlib.sha256(p.read_bytes()).hexdigest() for p in sorted(files)})
